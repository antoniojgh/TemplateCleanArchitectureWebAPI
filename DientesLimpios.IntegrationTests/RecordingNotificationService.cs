...
namespace DientesLimpios.IntegrationTests
{
    ...
    public sealed class RecordingNotificationService : INotificationService
    {
        ...
        private readonly ConcurrentBag<AppointmentRescheduledDTO> _rescheduledConfirmations = [];
        ...
        private readonly ConcurrentDictionary<Guid, byte> _failRescheduledOnce = new();
        ...
        public IReadOnlyCollection<AppointmentRescheduledDTO> RescheduledConfirmations => _rescheduledConfirmations;
        ...
        public void FailNextRescheduledFor(Guid appointmentId) => _failRescheduledOnce.TryAdd(appointmentId, 0);
        ...
        private Func<AppointmentRescheduledDTO, Task>? _beforeRescheduled;
        ...
        public void BeforeNextRescheduled(Func<AppointmentRescheduledDTO, Task> hook) =>
            Interlocked.Exchange(ref _beforeRescheduled, hook);
        ...
        public async Task SendAppointmentRescheduled(AppointmentRescheduledDTO appointment, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(appointment);

            // Removing the entry means the next attempt for this appointment succeeds.
            if (_failRescheduledOnce.TryRemove(appointment.Id, out _))
                throw new InvalidOperationException("Simulated SMTP failure.");

            var hook = Interlocked.Exchange(ref _beforeRescheduled, null);
            if (hook is not null)
                await hook(appointment);

            _rescheduledConfirmations.Add(appointment);
        }
